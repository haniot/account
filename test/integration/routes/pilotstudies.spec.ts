...
const userRepo: IHealthProfessionalRepository = container.get(Identifier.HEALTH_PROFESSIONAL_REPOSITORY)
...
describe('Routes: PilotStudies', () => {
    ...
    before(async () => {
            try {
                ...
                await userRepo.create(user).then(result => user.id = result.id)
