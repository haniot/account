    ...
    describe('fromJSON()', () => {
        context('when pass a complete json', () => {
            it('should return a complete pilot study model', () => {
                ...
                assert.propertyVal(result, 'total_health_professionals', DefaultEntityMock.PILOT_STUDY.total_health_professionals)
                assert.propertyVal(result, 'total_patients', DefaultEntityMock.PILOT_STUDY.total_patients)
    ...
    describe('toJSON()', () => {
        ...
        context('when does not have a health professionals or patients list', () => {
            it('should return a empty array for both parameters', () => {
                ...
                assert.propertyVal(result, 'total_health_professionals', DefaultEntityMock.PILOT_STUDY.total_health_professionals)
                assert.propertyVal(result, 'total_patients', DefaultEntityMock.PILOT_STUDY.total_patients)
                assert.lengthOf(result.health_professionals!, 0)
                assert.lengthOf(result.patients!, 0)
                assert.propertyVal(result, 'location', DefaultEntityMock.PILOT_STUDY.location)
            })
        })
    })
